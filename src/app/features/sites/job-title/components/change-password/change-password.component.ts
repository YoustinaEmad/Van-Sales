...
import { changePasswordViewModel } from '../../interfaces/job-title-view-model';
...
export class ChangePasswordComponent implements OnInit {
  ...
  constructor(private _router: Router, private _employeeService: EmployeeService, private _sharedService: SharedService,
  ...
  changePassword(): void {
    ...
    this._employeeService.changePassword(this.item).subscribe({
      next: (res) => {
        this.page.isSaving = false;
        this.page.responseViewModel = res;
        this._sharedService.showToastr(res);
        if (res.isSuccess) {
          this._router.navigate(['/sites/jobTitle/edit', this.id]);
        }
      },
      error: (err) => {
